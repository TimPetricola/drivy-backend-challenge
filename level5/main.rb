...
module ActionsManager
  # Stores a debit/credit action
  Action = Struct.new(:who, :type, :amount)

  def self.actions_for_rental(rental)
    [
      Action.new(:driver, :debit, rental.price_with_options),
      Action.new(:owner, :credit, rental.price - rental.commission),
      Action.new(:insurance, :credit, rental.insurance_fee),
      Action.new(:assistance, :credit, rental.assistance_fee),
      Action.new(
        :drivy, :credit,
        rental.drivy_fee + rental.deductible_reduction_price
      ),
    ]
  end
end
  ...
  def self.output
    ...
    JSON.pretty_generate({
      rentals: store.rentals.map do |rental|
        {
          id: rental.id,
          actions: ActionsManager.actions_for_rental(rental).map do |action|
            {
              who: action.who,
              type: action.type,
